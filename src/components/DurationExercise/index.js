...
function DurationInterface(props) {
    ...
    useEffect(() => {
        ...
        return () => clearInterval(interval); //when timer is no longer active (through reset), clear the interval so that it doesn't keep incrementing (1st part of reseting the timer)
      }, [status, time]);
